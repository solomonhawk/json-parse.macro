...
const { getSource, identifierToLiteral, objectToLiteral } = require('./helpers')

module.exports = createMacro(jsonParseMacro)

function jsonParseMacro({ references }) {
  references.default.forEach(reference => {
    if (!reference.parentPath.isCallExpression()) {
      throw new MacroError(
        `json-parse.macro should be used as function call, instead you have used it as part of "${reference.parentPath.node.type}".`
      )
    }
    ...
    if (!(argument.isObjectExpression() || argument.isIdentifier())) {
      throw new MacroError(
        `json-parse.macro only works with object literal values and identifiers, found "${argument.node.type}".`
      )
    }
    ...
    if (argument.isIdentifier()) {
      ...
      if (binding.referenced && binding.references === 1) {
        ...
        throw new MacroError(
          `json-parse.macro cannot optimize bindings that are referenced by other code. Ensure the variable passed to \`macro(..)\` is not referenced elsewhere.`
        )
      }
    }
  })
...
function replaceWithJSON(path, node) {
  path.parentPath.replaceWith(
    t.callExpression(
      ...
      [t.stringLiteral(JSON.stringify(getSource(node)))]
    )
  )
}
